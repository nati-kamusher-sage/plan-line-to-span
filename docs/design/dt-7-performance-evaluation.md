...
> Create, update, delete, `Query Benefit`, and `Query Employee` must use the index rather than scanning every stored benefit under normal operation.

This is a claim about *how cost grows*, not about any particular duration. A design that answers in 3 milliseconds is not thereby conformant, and one that takes 30 is not thereby in breach. The verifiable content is that cost must not grow in proportion to the number of stored benefits.
...
| Demo scale | Dominated by noise at hundreds of benefits | Unaffected |
...
The method: hold the query fixed, grow the benefit count geometrically, and compare the growth in comparisons against the growth in data.
...
An eightfold increase in stored benefits costs the scan exactly eight times more work and the index 1.4 times more. The method reaches the correct verdict for both, which is the evidence that it is measuring something.
...
| Volume | Dimensions | Values per dimension | Hierarchy depth | Benefits |
|---|---:|---:|---:|---:|
| V1 minimal | 1 | 5 | 2 | 10 |
...
The zero-dimensional model is excluded: it holds at most one benefit by DT-3's invariant, so growth is undefined and the question is meaningless.

## 6. Pass condition

For each volume, and for `Query Employee` and `Query Benefit`:

> Comparisons at 8N benefits shall be fewer than 4 times comparisons at N.
...
| Reporting | A table of volume, benefit count, comparisons, and growth ratio, plus the verdict. |
...
| DEC-46 | Judge OC 15.2 by growth rate against benefit count | That is what the clause actually constrains. |

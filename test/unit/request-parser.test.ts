...
test('AC-VAL-03: a duplicate object member is rejected as MALFORMED_REQUEST', () => {
  const raw = '{"contractVersion":"plan-line-to-span/v1","operation":"queryBenefit",' +
...
test('a duplicate top-level envelope member is rejected', () => {
  const raw = '{"contractVersion":"plan-line-to-span/v1","operation":"queryBenefit",' +
    '"operation":"createBenefit","payload":{"span":{}}}';
  assert.throws(() => parseRequest(raw), MalformedRequestError);
});

test('the same key name in sibling objects at the same depth is not a false positive', () => {
  const raw = '{"contractVersion":"plan-line-to-span/v1","operation":"createBenefit",' +
    '"payload":{"span":{"a":"1"},"formula":{"a":2}}}';
  assert.doesNotThrow(() => parseRequest(raw));
});

test('the same key name at a different nesting depth is not a false positive', () => {
  const raw = '{"contractVersion":"plan-line-to-span/v1","operation":"createBenefit",' +
    '"payload":{"span":{"location":"4"},"formula":{"location":"not-a-dimension-here"}}}';
  assert.doesNotThrow(() => parseRequest(raw));
});

test('structural-looking characters inside a string value do not confuse duplicate detection', () => {
  const raw = JSON.stringify({
    contractVersion: 'plan-line-to-span/v1', operation: 'createBenefit',
    payload: { span: { location: '4' }, formula: { note: 'a{b}c,d:e"f' } },
  });
  assert.doesNotThrow(() => parseRequest(raw));
});
...
test('an array of objects, each with their own keys, is not a false positive', () => {
  const raw = JSON.stringify({
    contractVersion: 'plan-line-to-span/v1', operation: 'createBenefit',
    payload: { span: { location: '4' }, formula: { list: [{ a: 1 }, { a: 2 }] } },
  });
  assert.doesNotThrow(() => parseRequest(raw));
});
...
test('AC-VAL-06: a request missing payload is rejected', () => {
  const raw = JSON.stringify({ contractVersion: 'plan-line-to-span/v1', operation: 'createBenefit' });
  assert.throws(() => parseRequest(raw), MalformedRequestError);
});

test('AC-VAL-06: a request with an undeclared top-level field is rejected', () => {
  const raw = JSON.stringify({
    contractVersion: 'plan-line-to-span/v1', operation: 'queryBenefit',
    payload: { span: {} }, bogus: 1,
  });
  assert.throws(() => parseRequest(raw), MalformedRequestError);
});

// ---- DEC-28/DEC-42: formula and format must NOT be judged here ----

test('a null formula is accepted by RequestParser; only FormulaValidator may reject it', () => {
  const raw = JSON.stringify({
    contractVersion: 'plan-line-to-span/v1', operation: 'createBenefit',
    payload: { span: {}, formula: null },
  });
...
test('an unsupported contractVersion is rejected', () => {
  const raw = JSON.stringify({
    contractVersion: 'plan-line-to-span/v2', operation: 'queryBenefit', payload: { span: {} },
  });
  assert.throws(() => parseRequest(raw), MalformedRequestError);
});
...
test('a numeric dimension value is rejected structurally', () => {
  const raw = JSON.stringify({
    contractVersion: 'plan-line-to-span/v1', operation: 'queryBenefit', payload: { span: { location: 4 } },
  });
  assert.throws(() => parseRequest(raw), MalformedRequestError);
});
...
test('queryEmployee requires a dimensions map, not a bare object', () => {
  const raw = JSON.stringify({
    contractVersion: 'plan-line-to-span/v1', operation: 'queryEmployee', payload: { location: '20' },
  });
  assert.throws(() => parseRequest(raw), MalformedRequestError);
});

test('updateBenefit with an extra replacementSpan field is rejected', () => {
  const raw = JSON.stringify({
    contractVersion: 'plan-line-to-span/v1', operation: 'updateBenefit',
    payload: { span: { location: '4' }, formula: {}, replacementSpan: { location: '20' } },
  });
  assert.throws(() => parseRequest(raw), MalformedRequestError);
});
...
test('a parsed queryEmployee request exposes payload.dimensions', () => {
  const parsed = parseRequest(JSON.stringify({
    contractVersion: 'plan-line-to-span/v1', operation: 'queryEmployee',
    payload: { dimensions: { location: '20' } },
  }));
  assert.equal(parsed.operation, 'queryEmployee');
  if (parsed.operation === 'queryEmployee') {
...
test('requestId is echoed when supplied and is optional', () => {
  ...
  const withoutId = parseRequest(JSON.stringify({
    contractVersion: 'plan-line-to-span/v1', operation: 'queryBenefit', payload: { span: {} },
  }));
